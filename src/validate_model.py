...
parser.add_argument('--imgsz', type=int, default='1024',
                    help='Specify the image size (width=height)')
parser.add_argument('--conf-thres', type=float, default='0.5',
                    help='Specify the confidence thresold for NMS')
parser.add_argument('--iou-thres', type=float, default='0.5',
                    help='Specify the IoU thresold for NMS')
...
image_paths = glob(join(_CURRENT_DIR, '..', 'dataset',
                        f'WIDER_{split_set}', 'images', '**', '*.jpg'), recursive=True)
...
def get_bboxes(img):
    ...
    inds = batched_nms(boxes, scores, torch.tensor(
        [0 for _ in range(scores.shape[0])]), iou_threshold=iou_thres)
...
with tqdm(total=len(image_paths)) as pbar:
    for img_path in image_paths:
        ...
        # show infos
        pbar.set_postfix(
            {'image': split(img_path)[1], 'n_labels': labels.shape[0]})
